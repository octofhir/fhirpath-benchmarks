...
impl RustRunner {
    pub fn new() -> Self {
        Self {
            base: BaseRunner::new("Rust".to_string()),
        }
    }
}

#[async_trait]
impl ImplementationRunner for RustRunner {
    fn name(&self) -> &str {
        &self.base.name
    }
    ...
    async fn run_tests(&self, config: &ImplementationConfig, impl_dir: &PathBuf, results_dir: &PathBuf) -> Result<TestResult> {
        ...
        crate::utils::cleanup_result_files(results_dir, "rust", "test_results")?;
        ...
        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Ok(self.base.create_error_test_result("rust", &stderr));
        }

        let pattern = "rust_test_results.json";
    ...
    async fn run_benchmarks(&self, config: &ImplementationConfig, impl_dir: &PathBuf, results_dir: &PathBuf) -> Result<BenchmarkResult> {
        ...
        crate::utils::cleanup_result_files(results_dir, "rust", "benchmark_results")?;
        ...
        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Ok(self.base.create_error_benchmark_result("rust", &stderr));
        }

        let pattern = "rust_benchmark_results.json";
        if let Some(result) = self.base.load_result_from_file::<BenchmarkResult>(results_dir, pattern).await? {
            return Ok(result);
        }

        Ok(self.base.create_error_benchmark_result("rust", "No results file generated"))
    }
}

impl RustRunner {
    fn parse_test_output(&self, output: &str) -> Result<TestResult> {
        let mut result = TestResult::new("rust".to_string(), crate::utils::get_system_info());
        ...
        for line in lines {
            if line.contains("✅") || line.contains("test result: ok") || line.contains("passed") {
                passed_count += 1;
                total_count += 1;
            } else if line.contains("❌") || line.contains("FAILED") || line.contains("failed") || line.to_lowercase().contains("error") {
                total_count += 1;
            }
        }
